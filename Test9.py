...
def NetworkModel(sigma, alpha):
    ...
    p_ecc = 0.8 # percentage of excitatory cells
    p_ini = 0.2 # percentage of inhibitory cells

    N_ecc = int(N_tot*p_ecc) # number of excitatory cells

    p_ecc_IB = 0 # percentage of IB
    p_ecc_RS = 1 # percentage of RS
    ...
    # :::::::::::::::::::::::::::::::::::::::    
    EccCellsList = [] # definition of the list containing excitatory cells
...
#    ampl=8/((np.sqrt(2)*np.sqrt(math.pi)*sigma/2)*math.erf(np.sqrt(2)/(2*sigma))*0.1*80)
    ...
    for i in range(N_ecc): 
        ...
        for j in NewArrPercEE:
            ...
            nc_E_E = h.NetCon(EccCellsList[i].soma(0.5)._ref_v,syn_AMPA,0,0,w_E_E,sec=EccCellsList[i].soma) 
            netcons_E_E.append(nc_E_E) 
            syn_E_E.append(syn_AMPA)

            # ----- NMDA ----- 
            syn_NMDA = h.Exp2SynM_NMDA(EccCellsList[j].soma(0.5))
            syn_NMDA.tau1 = 4
            syn_NMDA.tau2 = 40
            syn_NMDA.e = 0
            syn_NMDA.dd = 0.8
            ...
            nc_E_E = h.NetCon(EccCellsList[i].soma(0.5)._ref_v,syn_NMDA,0,0,w_E_E,sec=EccCellsList[i].soma) 
            netcons_E_E.append(nc_E_E) 
        ...
        for j in NewArrPercEI:
            ...
            nc = h.NetCon(EccCellsList[i].soma(0.5)._ref_v,syn_AMPA,0,0,w_E_I,sec=EccCellsList[i].soma)
            ...
            nc = h.NetCon(EccCellsList[i].soma(0.5)._ref_v,syn_NMDA,0,0,w_E_I,sec=EccCellsList[i].soma)
    ...
    """ Sections of the script related to the inhibitory cells
    """
    ...
    for i in range(N_ini): 
        ...
        # connections Inhib --> Inhib
        for j in NewArrPercII:
            # target cell is the j-th inhibitory cell
            syn = h.Exp2SynM(IniCellsList[j].soma(0.5))
            # prop Exp2SynM of inhibitory cell
            syn.tau1 =  0.1
            syn.tau2 = 3.0 #5.0
            syn.e = -70
            syn.dd = 0.8
    ...
    """ Recording of SpikeTimes from inhibitory cells useful for data processing """
    ...
    """ From this line are initialized the properties useful for the NETSTIM sections """
    p_ecc_stim = 1
    ...
    """Function to record electric potentials """
    def RecV(cell_element): 
        rec_v = h.Vector() 
        rec_v.record(cell_element.soma(0.5)._ref_v)
        return rec_v
    
    """ Properties of the simulation in NEURON """
    h.step(0.025)
    h.finitialize(-70*mV)
    h.continuerun(stim_dur*ms)    
    
    """ Saving spike times in python  """
    spike_times_python = [] 
